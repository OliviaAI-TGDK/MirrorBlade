...
namespace MB {
    ...
    //
    // - gain > 0 pulls toward the crease (underfold).
    // - gain < 0 pushes away.
    // - radius controls the influence width.
    // - priority controls application order (stable and deterministic).
    //
    // Thread-safe for concurrent Evaluate* while mutating via the API.
    // JSON helpers are implemented in the .cpp (depend on nlohmann/json there only).
    class LoomisUnderfold {
    public:
        struct Crease {
            std::string name;  // unique key
            double      pos = 0.0;   // center position of crease
            double      radius = 1.0;   // > 0; influence radius
        ...
        enum class Curve {
            ...
            Smooth,   // smoothstep: 3t^2 - 2t^3 over [0,1], mirrored as 1 - s
            Cosine,   // K(t) = 0.5*(1 + cos(pi*t)) for t∈[0,1], else 0
        ...
        // Evaluation ----------------------------------------------------
        // Evaluate the folded position for a single x.
        double Evaluate(double x) const;

        // Evaluate the total displacement: Evaluate(x) - x.
        double EvaluateDelta(double x) const;

        // Evaluate derivative d(Evaluate(x))/dx (useful for remapping densities).
        double EvaluateDerivative(double x) const;

        // Batched: xs/out are arrays of size n. out may alias xs.
        void EvaluateMany(const double* xs, double* out, size_t n) const;

        // JSON IO (implemented in .cpp; header avoids json dependency)
        // Format:
        // {
        //   "replace": false,              // optional
        //   "curve": "linear|smooth|cosine|hermite", // optional
        //   "creases":[
